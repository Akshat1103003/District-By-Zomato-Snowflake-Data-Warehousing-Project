...
SELECT '✅ DIM_VENUE created as Dynamic Table ' AS status;
...
-- Row count :
SELECT COUNT(*) ,venue_categories AS venue_count FROM DIM_VENUE group by venue_categories ;
...
SELECT
    ...
    TOTAL_SCREENS,
    TOTAL_BOOKINGS,
    SUPPORTS_PASS_DINING,
    SUPPORTS_PASS_MOVIES
FROM DIM_VENUE
...
SELECT
    ...
    ROUND(AVG(VENUE_RATING), 1) AS avg_rating
FROM DIM_VENUE
GROUP BY VENUE_TYPE
...
-- Pass support analysis :
SELECT
    VENUE_TYPE,
    SUM(CASE WHEN SUPPORTS_PASS_DINING THEN 1 ELSE 0 END) AS dining_enabled,
    SUM(CASE WHEN SUPPORTS_PASS_MOVIES THEN 1 ELSE 0 END) AS movies_enabled,
    SUM(CASE WHEN SUPPORTS_PASS_SNACKS THEN 1 ELSE 0 END) AS snacks_enabled,
    COUNT(*) AS total_venues
FROM DIM_VENUE
GROUP BY VENUE_TYPE
ORDER BY total_venues DESC;

-- Multi-category venues : 
SELECT
    VENUE_ID,
    VENUE_NAME,
    VENUE_CATEGORIES,
    TOTAL_BOOKINGS
FROM DIM_VENUE
WHERE VENUE_CATEGORIES LIKE '%,%'  -- Has multiple categories
ORDER BY TOTAL_BOOKINGS DESC
LIMIT 10;
...
SELECT
    COUNT(*) AS total_venues,
    SUM(CASE WHEN VENUE_NAME IS NULL THEN 1 ELSE 0 END) AS missing_name,
    SUM(CASE WHEN VENUE_CITY IS NULL THEN 1 ELSE 0 END) AS missing_city,
    SUM(CASE WHEN VENUE_TYPE IS NULL THEN 1 ELSE 0 END) AS missing_type,
    SUM(CASE WHEN LATITUDE IS NULL THEN 1 ELSE 0 END) AS missing_coordinates
FROM DIM_VENUE;

SELECT '🎉 DIM_VENUE COMPLETE - FIXED AGGREGATION & BRONZE ALIGNMENT!' AS message;
